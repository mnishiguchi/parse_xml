...
My plan for restructuring:
  Invokation
    mits = MitsParser.parse(json_hash)  #==> Sanitized feed for mits
...
class MitsParser
  ...
  # data - data data of any data type, typically hash, array, string
  # path  - always an array of strings
  ...
  def self.dig(data, path)
    ...
    node, remaining_path = path[0], path[1..-1]

    # "Array" is a special word we use to specify a node is an array.
    if node == ARRAY_NODE
      # Recurse on all the nodes in the array.
      data.map { |d| MitsParser.dig(d, remaining_path) }
    elsif data.is_a?(Hash) && data[node]
      # If data is a hash, recurse on the remaining path.
      MitsParser.dig(data[node], remaining_path)
    else
      []  #<== BAD return date instead

      # TODO: This is data is currrently ignored and set to [].
      # As soon as we include this data, our code will break.
      # data
    end
  end
